...
static void KEYBRD_Decode(uint8_t *pbuf)
{
  ...
  if (nbr_keys_new == 1) {
    ...
    /* Forward raw keycode + modifiers to BSP keyboard layer */
    extern void Keyboard_ReportCallback(uint8_t modifiers, uint8_t keycode);
    Keyboard_ReportCallback(pbuf[0], key_newest);
